...
module.exports.encode = function(uniqueInt, radix) {
	...
	var uniqueIntString = new Number(uniqueInt);
	var returnLength = startLength;
	
	checkMapping();
	
	if(uniqueIntString > new Number(9007199254740992)) {
		throw new Error('Integer can not be larger than 9007199254740992 due to limit in javascript');
	}

	switch(radix) {
		case 36:
			uniqueBaseString = uniqueIntString.toString(36);
			if(returnLength > 11) {
				returnLength = 11;
			}
			break;
		case 10:
			uniqueBaseString = uniqueIntString.toString(10);
			if(returnLength > 16) {
				returnLength = 16;
			}
			break;
		case 16:
			uniqueBaseString = uniqueIntString.toString(16);
			if(returnLength > 14) {
				returnLength = 14;
			}
			break;
